...
`--recursive` включает подпапки. Файлы обрабатываются по одному: лимита Telegram
и платных уровней нет. По умолчанию фото ограничены **24 мегапикселями**, чтобы
...
Весь код обработки находится внутри навыка. Сервер, база данных и токен бота
не нужны. Оба агента и CLI используют одну реализацию.
...
Тесты создают материалы сами. Личных фото, ключей, логов и базы бота в репозитории
нет. Перед изменением алгоритма прочитай [CONTRIBUTING.md](CONTRIBUTING.md).
